...
#     orig           => undef,          ## a File obj for the orig url
     orig_url_name  => undef,          ## incoming url_name
    ...
    if( $args->{'is_error_template'} )
    {
	$page->{'error_template'} = 1;
    }
...
sub is_index
{
    die "not implemented" unless $_[0]->site;
...
sub desig
{
    ...
    my $sys_path = $page->{'sys_name'} || '<unknown>';
    return sprintf "%s (%s)", $sys_path, $page->path_slash;
}
...
sub url_path_tmpl
{
    confess "not implemented" unless $_[0]->site;
...
sub path_tmpl
{
    ...
    confess "not implemented" unless $page->site;
...
sub set_headers
{
    ...
    confess "wrong turn" unless $page->site;
...
sub add_header
{
    confess "wrong turn" unless $_[0]->site;
