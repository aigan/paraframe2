...
function setFontSize(fontVal) {
	...
	createCookie('meyerweb-baseTextSize', fontVal, 365);
	docBase[0].style.fontSize = docSize;
	if (fontSet) {fontSet.value = fontVal;}
}

window.onload = function(e) {
  var cookie = readCookie('meyerweb-themeName');
  var title = cookie ? cookie : getPreferredStyleSheet();
  setActiveStyleSheet(title);
  var cookie2 = readCookie('meyerweb-baseTextSize');
  if (cookie2) {setFontSize(cookie2);}
}

window.onunload = function(e) {
  var title = getActiveStyleSheet();
  createCookie('meyerweb-themeName', title, 365);
}
